...
Plans:
- [x] 06-01-PLAN.md — Set DM cookie on campaign creation and display join link on dashboard
- [x] 06-02-PLAN.md — Home page returning DM redirect and end-to-end human verification
...
- No `secure: true` on httpOnly cookies (`dm_secret`, `player_id`) — Next.js dev is HTTP; Vercel enforces HTTPS at platform level
- Silent fallthrough on stale `dm_secret` cookie — handles DB reset gracefully; DM can create new campaign without error
- `JoinForm` extracted to separate file — clean `use client` boundary from server component routing logic
- `redirect()` as final statement in `registerPlayer` (not inside try/catch) — Next.js `redirect()` throws internally
- Cookie-based identity replaces per-player invite links — single join link lowers friction, no individual link management needed

**Issues Resolved:**

- All v1.0 field references eliminated cleanly across plans 05-02 and 05-03; build was clean on first run in 05-04
